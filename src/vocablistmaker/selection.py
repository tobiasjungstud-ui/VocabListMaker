...
def _diversity_score(c: Candidate, pos_counts: Counter, topic_counts: Counter) -> float:
...
def _greedy_pick(pool: list[Candidate], target: int, seed: list[Candidate] | None = None) -> list[Candidate]:
    ...
    while remaining and len(chosen) < target:
        best = max(remaining, key=lambda c: (_diversity_score(c, pos_counts, topic_counts), c.difficulty))
...
def select_words(
    ...
    """
    result = Selection()
    result.chosen = _greedy_pick(usable, target)
    ...
    if len(result.chosen) < target and fallback:
        ...
        for flag, label in ladder:
            ...
            extras.sort(key=lambda c: (-c.learning_value, -c.difficulty))
            for c in extras:
                if len(result.chosen) >= target:
                    break
